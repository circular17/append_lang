// Grammar to preprocess custom PEG file
...
ReturnError
    = "=>" _ "error" _ "(" _ error:StringLiteral _ ")" {
        return `{ error(${error}) }`
    }

Identifier
  = head:IdentifierStart tail:IdentifierPart* { return text() }

IdentifierStart
  = UnicodeLetter
  / "$"
  / "_"
  / "\\" c:UnicodeEscapeSequence { return c }
...
CharacterClassMatcher "character class"
  = "[" inverted:"^"? parts:CharacterPart* "]" ignoreCase:"i"? { return text() }
...
HeadTail
    = head:Identifier _ "::" _ tail:Identifier {
...
Code
    = tokens:CodeToken* { return tokens.join("") }
...
StringLiteral "string"
    = '"' chars:DoubleStringCharacter* '"' { return text() }
    / "'" chars:SingleStringCharacter* "'" { return text() }
...
NonEscapeCharacter
    = !(EscapeCharacter / LineTerminator) c:. { return c }
